...
export function processMessage(magickRequest) {
...
function integrateWasmJS() {
    ...
    var wasmBinaryFile = chrome.runtime.getURL("js/lib/magick/magick.wasm");
    ...
    function doNativeWasm(global, env, providedBuffer) {
        ...
        if (!Module["wasmBinary"] && typeof WebAssembly.instantiateStreaming === "function" && !isDataURI(wasmBinaryFile) && typeof fetch === "function") {
            ...
            //     .catch(error => {
            //         err("Failed to load and instantiate Wasm: " + error);
            //         abort(error);
            //     });


            let wasmBinaryFile1 = chrome.runtime.getURL("js/lib/magick/magick.wasm");

            WebAssembly.instantiateStreaming(fetch(wasmBinaryFile), info).then(receiveInstantiatedSource).catch((function (reason) {
...
function ___syscall180(which, varargs) {
    ...
    try {
        var stream = SYSCALLS.getStreamFromFD(), buf = SYSCALLS.get(), count = SYSCALLS.get(),
            zero = SYSCALLS.getZero(), offset = SYSCALLS.get64();
...
function ___syscall324(which, varargs) {
    SYSCALLS.varargs = varargs;
    try {
        var stream = SYSCALLS.getStreamFromFD(), mode = SYSCALLS.get(), offset = SYSCALLS.get64(),
            len = SYSCALLS.get64();
        assert(mode === 0);
